...
from pre_gene.predict import pre_gene
from pre_cliff.pre_cliff import *


class DRDataset():
    def __init__(self, dataset_name=None, drug_neighbor_num=15, disease_neighbor_num=15,feature_topk=20,seed=42):
        ...
        if dataset_name=="lrssl":
            ...
            old_data = load_mat(dataset_name)
...
class Dataset():
    def __init__(self, dataset, mask, fill_unkown=True, stage="train", **kwargs):
        ...
        # self.pos_weight = neg_num/pos_num
        self.smi = dataset.smi

        self.dat = dataset.dataset_name
    ...
    def get_smi(self):
        seq = [smiles_to_sequence(i) for i in self.smi["smiles"].tolist()]
        return torch.tensor(seq, dtype=torch.long)
        
    def get_atom_bond_graph(self):

        atom_feat = []
        atom_edge = []
        atom_attr = []

        for smi in self.smi["smiles"].tolist():
            feat, edge_index, edge_attr = create_atom_bond_graph(smi)[0].get_atom_feature()

            atom_feat.append(torch.tensor(feat, dtype=torch.float))
            atom_edge.append(torch.tensor(edge_index, dtype=torch.long))
            atom_attr.append(torch.tensor(edge_attr, dtype=torch.float))

    # batch for atoms
        batch_atom_list = []
        for i, h in enumerate(atom_feat):
            num_nodes = h.size(0)
            batch_atom_list.append(torch.full((num_nodes,), i, dtype=torch.long))

        batch_atom = torch.cat(batch_atom_list, dim=0)
    ...
    # bonds
        ...
        for smi in self.smi["smiles"].tolist():
            feat, index = create_atom_bond_graph(smi)[1].get_bond_feature()

            bonds_feat.append(torch.tensor(feat, dtype=torch.float))
            bond_index.append(torch.tensor(index, dtype=torch.long))

    # batch for bonds
        batch_bond_list = []
        for i, h in enumerate(bonds_feat):
            num_bonds = h.size(0)
            batch_bond_list.append(torch.full((num_bonds,), i, dtype=torch.long))

        batch_bond = torch.cat(batch_bond_list, dim=0)

    # concat all graphs
        atom_feat = torch.cat(atom_feat, dim=0)
        atom_edge = torch.cat(atom_edge, dim=1)
        atom_attr = torch.cat(atom_attr, dim=0)

        bonds_feat = torch.cat(bonds_feat, dim=0)
        bond_index = torch.cat(bond_index, dim=1)
    ...
    def get_egnn(self):
        three_d_g = [smiles_to_egnn(i) for i in self.smi["smiles"].tolist()]
        h_3d = [i[0] for i in three_d_g]
        x_3d = [i[1] for i in three_d_g]
        edge_index_3d = [i[2] for i in three_d_g]
        edge_attr_3d = [i[3] for i in three_d_g]

        batch_3d_list = []
        for i, h in enumerate(h_3d):
            num_nodes = h.size(0)
            batch_3d_list.append(torch.full((num_nodes,), i, dtype=torch.long))
        batch_3d = torch.cat(batch_3d_list, dim=0) 
        
        h_3d   = torch.cat(h_3d, dim=0)
        x_3d   = torch.cat(x_3d, dim=0) 
        edge_index_3d  = torch.cat(edge_index_3d, dim=1) 
        edge_attr_3d  = torch.cat(edge_attr_3d, dim=0)

        
        return h_3d, x_3d, edge_index_3d, edge_attr_3d, batch_3d
    def get_genes(self):
        paths = {
    "lrssl": "./dataset/drimc/drugs.csv",
    "Cdataset": "./dataset/Cdataset_drugs.csv",
    "Gdataset": "./dataset/Gdataset_drugs.csv",
    "hdvd":"./dataset/hdvd/drugs.csv"}       
        dataset_path = paths.get(self.dat) 
        data = pre_gene(dataset_path)
        return data
#
#
    def get_cliffs(self):
    ...
    "hdvd":"./dataset/hdvd/drugs.csv"} 
            dataset_path = paths.get(self.dat)
            dat = MoleculeDataset(dataset_path)
            loader = DataLoader(
    dat.data_all,
    batch_size=32,
    shuffle=False
            ...
            args = get_args()
            device = "cuda:0"
            weight_list = os.listdir("./pre_cliff/weight")
            ...
            for weight_p in weight_list:   # 外层循环（多个模型 / 多个权重）
                ...
                for batch in loader:
                    ...
                    with torch.no_grad():
                        y_pred = predict(os.path.join("./pre_cliff/weight", weight_p), args, batch, device)
...
def load_DRIMC(root_dir="dataset/drimc", name="c", reduce=True):
    ...
    smi = pd.read_csv(os.path.join(root_dir, "drugs.csv"), sep=",")
    if reduce:
...
def load_mat(dataset_name):
    data = scio.loadmat(f"dataset/{dataset_name}.mat")
    drug = data["drug"]
    disease = data["disease"]
    Wrname = data["Wrname"]
    Wdname = data["Wdname"]
    didr = data["didr"]
    smi = pd.read_csv(f'dataset/{dataset_name}_drugs.csv', sep=",")
    return {"drug":drug,
            "disease":disease,
            "Wrname":Wrname,
            "Wdname":Wdname,
