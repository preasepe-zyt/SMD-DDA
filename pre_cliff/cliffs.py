...
def mmp_is_cliff(mmpdb_out_df, smiles, dist_thre):
    """
    Get ACs based on matched molecular pair.
    """
    data = pd.DataFrame({'smiles': smiles})
    merged1 = mmpdb_out_df.merge(data.rename(columns={'smiles': 'smi_1'}), on='smi_1')
    merged2 = merged1.merge(data.rename(columns={'smiles': 'smi_2'}), on='smi_2')
