...
class MolTensorizer(object):
    ...
    def tensorize(self, smile: Union[str, Mol]) -> Data:
        ...
        num_nodes = torch.FloatTensor(atoms_feature).size(0)
        num_bonds = torch.FloatTensor(bonds_feature).size(0)
        bond_batch = torch.zeros(num_bonds, dtype=torch.long)
        num_3d_nodes = torch.FloatTensor(h).size(0)
        ...
        if len(atoms_feature) == 0 or len(edge_index) == 0 or len(bonds_feature) == 0 or len(h) == 0:
                ...
                atom_g, bond_g = self.create_atom_bond_graph(smi)
                atoms_feature, edge_index, edge_attr = atom_g.get_atom_feature()
                bonds_feature, bond_index = bond_g.get_bond_feature()
                three_d_g = self.smiles_to_egnn(smi)
                h, x, edges_3d, edge_attr_3d = three_d_g
                num_nodes = torch.FloatTensor(atoms_feature).size(0)
                num_bonds = torch.FloatTensor(bonds_feature).size(0)
                bond_batch = torch.zeros(num_bonds, dtype=torch.long)
                num_3d_nodes = torch.FloatTensor(h).size(0)
                batch_3d = torch.zeros(num_3d_nodes, dtype=torch.long)
        data = Data( atoms_features=torch.FloatTensor(atoms_feature),
                    ...
                    batch_3d = batch_3d)
        data.num_nodes = torch.FloatTensor(atoms_feature).size(0)
        return data
