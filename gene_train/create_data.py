    ...
    def process_data(self, smi, gene, atom_graph, bond_graph, smile_sequences, three_d_graph):
        ...
        for i in range(data_len):
            try:
                ...
                smiles = smi[i]
                genes = gene[i]
                def safe_get_atom(smiles):
                    try:
                        return atom_graph[smiles].get_atom_feature()
                    except:
                        return atom_graph[FALLBACK_SMILES].get_atom_feature()
                def safe_get_bond(smiles):
                    try:
                        return bond_graph[smiles].get_bond_feature()
                    except:
                        return bond_graph[FALLBACK_SMILES].get_bond_feature()
                def safe_get_seq(smiles):
                    try:
                        return smile_sequences[smiles]
                    except:
                        return smile_sequences[FALLBACK_SMILES]
                def safe_get_3d(smiles):
                    try:
                        return three_d_graph[smiles]
                    except:
                        return three_d_graph[FALLBACK_SMILES]
                atoms_feature, edge_index, edge_attr = safe_get_atom(smiles)
                bonds_feature, bond_index = safe_get_bond(smiles)
                seq = safe_get_seq(smiles)
                h, x, edges_3d, edge_attr_3d = safe_get_3d(smiles)
                GCNData = DATA.Data(
            smiles=smiles,
