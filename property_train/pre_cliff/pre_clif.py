...
class MoleculeDataset:
    def __init__(
        ...
        self.featurize_data()
        self.working_path = "/home/zyfone/hard-disk/zyt/property-main/pre_cliff/"
    def get_cliffs(self,
...
def predict(weight_p,args, data, device):
    ...
    #cliffs = torch.cat((data.cliff.cpu().reshape(-1, 1), output.cpu().detach().reshape(-1, args.num_classes)))
    return  y_pred, mol_f

if __name__ == "__main__":
    dat = MoleculeDataset("/home/zyfone/hard-disk/zyt/property-main/pre_cliff/Cdataset_drugs.csv")
    loader = DataLoader(
    dat.data_all,
    batch_size=32,
    shuffle=False
    ...
    for weight_p in weight_list:   # 外层循环（多个模型 / 多个权重）
        ...
        for batch in loader:
            with torch.no_grad():
                y_pred, mol_f = predict(os.path.join("./weight", weight_p), args, batch, device)
                print( weight_p)
                if "BBBP" in weight_p or "Clintox" in weight_p or "SIDER" in weight_p:
                   y_pred = torch.sigmoid(y_pred)
                mol_f_list.append(mol_f )
                pre_list.append(y_pred)
        
        pre_list = torch.cat(pre_list, dim=0)
        mol_f_list  = torch.cat(mol_f_list , dim=0)
        torch.save(mol_f_list, f"{weight_p}_mol_f.pt")
        
        pre_all.append(pre_list)
    pre_all = torch.cat(pre_all, dim=1)
    print(pre_all.shape)
    print(pre_all)