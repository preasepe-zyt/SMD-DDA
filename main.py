...
def train(config, model_cls=SMD_DDA):
    if config.dataset_name == "lrssl":
           config.seed = 666
    if config.dataset_name == "Gdataset":
           config.seed = 888
    if config.dataset_name == "Cdataset":
           config.seed = 666
    if config.dataset_name == "hdvd":
           config.seed = 888
...
def parse(print_help=False):
    ...
    parser.add_argument("--model", default="SMD_DDA", type=str)
    ...
    parser = SMD_DDA.add_model_specific_args(parser)
